...
(async () => {
  ...
  try {
    ...
    if (rows.length === 0) {
      throw new Error(Column . not found in schema .);
    }
    ...
    if (!match) {
      console.log(Column . is not ENUM. Skipping.);
      process.exit(0);
    }
    ...
    const enumSqlList = newEnumValues.map(v => \'\').join(',');
    const alterSql = ALTER TABLE \${targetTable}\ MODIFY \${targetColumn}\ ENUM() NOT NULL DEFAULT '';
